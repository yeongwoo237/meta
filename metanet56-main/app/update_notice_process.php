...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id'])) {
    ...
    // UPDATE SQL 실행
    $sql = "UPDATE notices SET title = ?, content = ?, author = ?, is_active = ?, category = ? WHERE id = ?";
    $stmt = $conn_azure->prepare($sql);
    $stmt->bind_param("sssisi", $title, $content, $author, $is_active, $category, $id);
